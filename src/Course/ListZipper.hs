...
toList ::
  ListZipper a
  -> List a
toList (ListZipper l x r) = foldLeft (flip (:.)) (x :. r) l
...
findLeft ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findLeft p lz@(ListZipper _ x _) | p x = IsZ $ lz
findLeft _ (ListZipper Nil _ _) = IsNotZ
findLeft p (ListZipper (y:.ys) x r) = findLeft p $ ListZipper ys y (x :. r)
...
findRight ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findRight p lz@(ListZipper _ x _) | p x = IsZ $ lz
findRight _ (ListZipper _  _ Nil) = IsNotZ
findRight p (ListZipper l x (y :. ys)) = findRight p $ ListZipper (x :. l) y ys
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop (ListZipper Nil x r) = let y :. ys = reverse $ x :. r
                                    in  ListZipper ys y Nil
moveLeftLoop (ListZipper (y :. ys) x r) = ListZipper ys y (x :. r)
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop (ListZipper l x Nil) = let y :. ys = reverse $ x :. l
                                     in ListZipper Nil y ys
moveRightLoop (ListZipper l x (y:.ys)) = ListZipper (x :. l) y ys
...
moveLeft (ListZipper (y:.ys) x r) = IsZ $ (ListZipper ys y (x:.r))
...
moveRight (ListZipper l x (y:.ys)) = IsZ $ (ListZipper (x:.l) y ys)
...
swapLeft (ListZipper (y:.ys) x r) = IsZ $ ListZipper (x:.ys) y r
...
swapRight (ListZipper l x (y:.ys)) = IsZ $ ListZipper l y (x :. ys)
...
moveLeftN ::
  ...
  -> MaybeListZipper a
moveLeftN 0 l = IsZ l
moveLeftN n l | n < 0 = moveRightN (-n) l
              | otherwise = case moveLeft l of 
                            IsNotZ -> IsNotZ
...
moveRightN n l | n < 0 = moveLeftN (-n) l
               | otherwise = case moveRight l of
                             IsNotZ -> IsNotZ
                             IsZ z -> moveRightN (n - 1) z
...
moveLeftN' n l = case moveLeftN n l of
                      IsNotZ -> Left . length . (if n < 0 then rights else lefts) $ l
                      (IsZ rt) -> Right rt
...
instance Apply ListZipper where
  (<*>) (ListZipper l1 f r1) (ListZipper l2 x r2) = ListZipper (zipWith ($) l1 l2) (f x) (zipWith ($) r1 r2)
